...
def getMTNServiceSettings():
    ...
    data.rica_password = get_decrypted_password('MTN Services Settings', 'MTN Services Settings', 'rica_password',
                                                False)
    return data
...
def GetInvoice(invoice, qty, itemcode, amount):
    if invoice == 'Sales':
        data = frappe.db.sql("""select name, parent, idx from `tabSales Invoice Item` where qty=%s and item_code=%s""",
                             (qty, itemcode))
